...
class Banco : Imprimivel {
    ...
    fun procurarConta(numeroConta:Int): ContaBancaria? {
        ...
        var IndiceEncontrado = verificaLista(numeroConta)

        if(verificaLista(numeroConta) >= menorIndicePossivel ){
            return listaConta[IndiceEncontrado]
        }else{
            return null
        }
                ...
                when(resposta){
                    ...
                    3 -> {
                        ...
                        if(this.verificaLista(valorNumeroConta) >= 0){
                            listaConta[menorIndicePossivel].transferir(valorTransferencia,listaConta[valorNumeroConta])
                        }else{
                    ...
                    4 -> relatorio.gerarRelatorio(listaConta[menorIndicePossivel])
